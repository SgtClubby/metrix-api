...
class PacketHandler {
    ...
    sendGotReady(remote) {
        let packet = this.makePacket("", PacketType.GOTREADY);
        this.socket.send(packet, 57021, "192.168.1.122");
    }
    ...
    readyHandler(remote, packet) {
        this.sendGotReady(remote)
        return console.log(`Remote: ${remote} Packer: ${packet}`);
    }
    ...
    handleHeartbeat(remote) {
        ...
        console.log("Got heartbeat from " + remote.address)
    ...
    handlePacket(packet, remote) {
        ...
        switch (type) {
            ...
            case PacketType.READY:
                this.readyHandler(packet, remote)
                break;
        }
    }
}
